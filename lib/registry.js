...
export function upsertManagedPatch(patchText, registry) {
  ...
  if (!foreignOp && !foreignId) {
    ...
    // 含外来内容：按块处理（块 = 从 "- insert:" 到下一个顶层条目/文件结束）
    // 插入块的子条目必然缩进；块终止于下一个顶格非空行（顶层条目或注释），
    // 避免把 insert 块之后的外来顶层条目（如 meow-memory）误并入本块，
    // 否则整段会被判为“外来块”保留、末尾再追加生成块 → 重复 id。
    ...
    while (i < lines.length) {
      if (lines[i].trim() === "- insert:") {
        ...
        while (j < lines.length) {
          ...
          if (trimmed === "") { block.push(line); j++; continue }
          if (line.startsWith(" ") || line.startsWith("\t")) { block.push(line); j++; continue }
          break
        }
        let idCount = 0
        let ourIdCount = 0
...
// ── 备份保留策略（v0.2.4）─────────────────────────────────────────
// 备份不再无限堆积；历史备份中的 Bearer Token 统一打码。
// 最新的一个备份保持原文——它是唯一的即时回滚点，必须能原样恢复；
// 更早的备份只用于"当时改了什么"的查阅，不是回滚点，故脱敏。
export const BACKUP_KEEP = 5

// 两种形态都要覆盖：DSH 物化产出 `Authorization: Bearer xxx`（无引号），
// 本插件 generatePatchLines 经 yamlScalar 产出 `Authorization: "Bearer xxx"`（有引号）。
const TOKEN_RE = /(Authorization:\s*"?Bearer\s+)([A-Za-z0-9._~+/=-]{8})[A-Za-z0-9._~+/=-]*/g
const REDACT_MARK = "…(redacted)"

/**
 * 把 Authorization 行的 Token 打码。**按行处理且跳过已打码行**——否则正则每次
 * 都会在已有标记前再匹配一次，导致每次调用都追加一个标记（文件持续变大）。
 */
export function redactTokens(text) {
  return String(text ?? "").split("\n").map(function (line) {
    if (line.indexOf(REDACT_MARK) !== -1) return line
    return line.replace(TOKEN_RE, "$1$2" + REDACT_MARK)
  }).join("\n")
}

/** 按时间序（文件名时间戳后缀）列出本插件产生的备份。 */
export function listBackups(target) {
  const dir = dirname(target)
  const prefix = basename(target) + ".mcp-bak-"
  let names
  try { names = readdirSync(dir) } catch (e) { return [] }
  return names
    .filter(function (n) { return n.indexOf(prefix) === 0 })
    .sort()
    .map(function (n) { return join(dir, n) })
}

/**
 * 收敛备份目录：只保留最近 keep 个；删除更早的，并把除最新一个以外的备份脱敏。
 * @returns {{removed:number, redacted:number, kept:number}}
 */
export function pruneBackups(target, keep) {
  const limit = (typeof keep === "number" && keep > 0) ? keep : BACKUP_KEEP
  const all = listBackups(target)
  const cut = Math.max(0, all.length - limit)
  const stale = all.slice(0, cut)
  const kept = all.slice(cut)
  let removed = 0
  let redacted = 0
  for (const p of stale) { try { unlinkSync(p); removed++ } catch (e) {} }
  for (let i = 0; i < kept.length - 1; i++) {
    try {
      const before = readFileSync(kept[i], "utf8")
      const after = redactTokens(before)
      if (after !== before) { writeFileSync(kept[i], after, "utf8"); redacted++ }
    } catch (e) {}
  }
  return { removed: removed, redacted: redacted, kept: kept.length }
}
